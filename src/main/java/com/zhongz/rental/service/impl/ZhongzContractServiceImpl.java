...
public class ZhongzContractServiceImpl implements IZhongzContractService
{
	...
	public Result<ZhongzContract> selectZhongzUserContractList(ZhongzUserOrderParam zhongzUserOrderParam)
	{
		...
		return new Result<>(zhongzUserOrderParam.getPageNum(),zhongzUserOrderParam.getPageSize(),totalNum,contracts);
	}
	...
	public Result<ZhongzContract> selectZhongzUserContractAllList(ZhongzUserOrderParam zhongzUserOrderParam)
	{
		...
		List<ZhongzContract> contracts = zhongzContractMapper.selectZhongzContractByTenantId(user.getId(),null,null);
		Integer totalNum = contracts.size();
		if(totalNum == 0) {
			return new Result<>(zhongzUserOrderParam.getPageNum(),zhongzUserOrderParam.getPageSize(),0,new ArrayList<>());
		}
		contracts = zhongzContractMapper.selectZhongzContractByTenantId(user.getId(),zhongzUserOrderParam.getLimitStart(),zhongzUserOrderParam.getLimitNum());
		if(contracts.size() == 0) {
			return new Result<>(zhongzUserOrderParam.getPageNum(),zhongzUserOrderParam.getPageSize(),totalNum,new ArrayList<>());
		}
		if(contracts != null && contracts.size() > 0) {
