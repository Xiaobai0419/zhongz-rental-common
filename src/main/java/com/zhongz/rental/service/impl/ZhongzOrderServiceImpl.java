...
public class ZhongzOrderServiceImpl implements IZhongzOrderService
{
	...
	public ZhongzContract selectZhongzOrderById(String id)
	{
		...
		ZhongzOrder order = null;
		try {
			order = orderHashOperations.get(orderKey,id);
		}catch ( Exception e) {
		...
		ZhongzContract contract = null;
		if(order != null) {
			//缓存获取合同、用户、房屋，没有降级去库查询，注意合同、用户和房屋信息更新时，更新同一个key中对应信息，并整体更新缓存过期时间
			try {
				contract = contractHashOperations.get(contractKey,order.getContractId());
			}catch (Exception e) {
			...
			if(contract != null) {
				ZhongzHouse house = null;
				try {
					house = houseHashOperations.get(houseKey,contract.getHouseId());
				}catch (Exception e) {
				...
				ZhongzUser user = null;
				try {
					user = userHashOperations.get(userKey,contract.getTenantId());
				}catch (Exception e) {
...
//				order.setContract(contract);
	...
	public List<ZhongzOrder> selectZhongzOrderList(ZhongzOrder zhongzOrder)
	{
		...
		if(orders.size() > 0) {
			...
			HashOperations<String, String, ZhongzContract> contractHashOperations = redisTemplate.opsForHash();
			List<ZhongzContract> contracts = null;
			try {
				contracts = contractHashOperations.multiGet(contractKey,contractIds);
			}catch (Exception e) {

			}
			if(contracts == null || contracts.size() == 0 || contracts.contains(null)) {
				//完全没缓存，一次性批量查
			...
			//查询关联房屋，缓存
			HashOperations<String, String, ZhongzHouse> houseHashOperations = redisTemplate.opsForHash();
			List<ZhongzHouse> houses = null;
			try {
				houses = houseHashOperations.multiGet(houseKey,houseIds);
			}catch (Exception e) {

			}
			if(houses == null || houses.size() == 0 || houses.contains(null)) {
				//完全没缓存，一次性批量查
			...
			//查询关联租客，缓存
			HashOperations<String, String, ZhongzUser> userHashOperations = redisTemplate.opsForHash();
			List<ZhongzUser> users = null;
			try {
				users = userHashOperations.multiGet(userKey,userIds);
			}catch (Exception e) {

			}
			if(users == null || users.size() == 0 || users.contains(null)) {
				//完全没缓存，一次性批量查
	...
	{
		//每次重新去数据库获取
		List<ZhongzOrder> orders = zhongzOrderMapper.selectZhongzOrderRenewList(zhongzOrder);
		if(orders.size() > 0) {
			Map<String,ZhongzOrder> orderMap = new HashMap<>();
			List<String> contractIds = new ArrayList<>();
			for(ZhongzOrder order : orders) {
	...
	public Result insertZhongzOrder(ZhongzUserOrderParam zhongzUserOrderParam)
	{
		...
		contract.setMobile(user.getMobile());
